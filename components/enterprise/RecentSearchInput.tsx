    ...
    <div ref={containerRef} className={`relative flex-1 ${className}`}>
      ...
      >
        ...
        <input
          ...
          className="w-full bg-transparent text-base md:text-[13px] font-medium outline-none placeholder:text-zinc-500"
        />
      </label>
      ...
      {isOpen && history.length > 0 && (
        <div
          className={`absolute left-0 right-0 top-full z-50 mt-1 overflow-hidden rounded-xl border shadow-xl backdrop-blur-md ${
            isLight
              ? 'border-zinc-200 bg-white text-black shadow-zinc-300/50'
              : 'border-zinc-800 bg-zinc-950 text-white shadow-black/80'
          }`}
        >
          <div className="flex items-center justify-between border-b px-3 py-1.5 text-[10.5px] font-bold uppercase tracking-wider text-zinc-400 border-inherit">
          ...
          <div className="py-1">
            {history.slice(0, 5).map((term, index) => (
              <div
                ...
                className={`group flex items-center justify-between px-3 py-2 text-[12.5px] font-semibold cursor-pointer transition ${
                  isLight
              ...
              >
                ...
                <button
                  ...
                  className="rounded p-1 text-zinc-400 opacity-0 transition hover:bg-zinc-800 hover:text-white group-hover:opacity-100"
